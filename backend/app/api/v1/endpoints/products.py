...
@router.get("/alerts")
async def stock_alerts(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    result = await db.execute(select(Product).where(Product.stock_status.in_(["low", "critical"])))
    products = result.scalars().all()
...
@router.post("/sales")
async def record_sale(data: SaleCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    purchase = Purchase(client_id=data.client_id, product_id=data.product_id, quantity=data.quantity, unit_price=data.unit_price, total=data.quantity * data.unit_price)
    db.add(purchase)
    await recalculate_stock_status(product, db)
    await db.commit()
