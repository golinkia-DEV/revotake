...
const EXAMPLES =
  "Ejemplos por rubro: peluquería (Cortes, Coloración), clínica (Consultas, Exámenes), taller (Mantención, Diagnóstico), estética (Facial, Corporal).";
...
export default function SchedulingServicesMenuPage() {
  ...
  const createMut = useMutation({
    mutationFn: (body: Record<string, unknown>) => api.post("/scheduling/services", body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["scheduling-services"] });
      setShowForm(false);
      toast.success("Servicio creado");
    },
    onError: (e: unknown) => {
      const d = (e as { response?: { data?: { detail?: string } } })?.response?.data?.detail;
      toast.error(typeof d === "string" ? d : "No se pudo crear (¿admin de tienda?)");
    },
  });

  const patchMut = useMutation({
    mutationFn: ({ id, body }: { id: string; body: Record<string, unknown> }) =>
      api.patch(`/scheduling/services/${id}`, body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["scheduling-services"] });
      setEditing(null);
      ...
      <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          ...
          <p className="max-w-2xl text-sm text-slate-500">
            Armá el catálogo que ven tus clientes al reservar: agrupá por categorías libres (cualquier rubro), orden y
            precios. Lo público respeta solo servicios activos.
          </p>
        </div>
