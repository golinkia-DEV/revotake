      ...
      await api.patch(
        `/stores/${configStore.id}`,
        {
          settings: {
            ...
            agenda: { default_duration_minutes: durationMin },
          },
        },
  ...
  return (
    <AppLayout>
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white mb-1">Tiendas</h1>
          <p className="text-gray-400">Cada tienda tiene su propia agenda, stock, clientes e IA.</p>
        </div>
      ...
      {showForm && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6 mb-8 max-w-xl">
          <h2 className="font-semibold text-white mb-4">Registrar tienda</h2>
          <p className="text-sm text-gray-500 mb-4">Elige el tipo de negocio: se cargará una plantilla de configuración (IA, agenda, stock).</p>
          <div className="space-y-4">
            <input value={name} onChange={(e) => setName(e.target.value)} className="input-field" placeholder="Nombre de la tienda *" />
            <div>
              <label className="text-xs text-gray-400 mb-2 block">Tipo de negocio</label>
        ...
        {(stores?.items as StoreItem[] | undefined)?.map((s) => (
          <motion.div key={s.id} layout className="glass-card-hover p-6 flex flex-col gap-4">
            <div className="flex items-start gap-3">
              <div className="w-12 h-12 rounded-xl bg-brand-600/30 flex items-center justify-center flex-shrink-0">
                <Building2 className="w-6 h-6 text-brand-300" />
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-white text-lg">{s.name}</h3>
                <p className="text-sm text-gray-500">{s.store_type_name}</p>
                <span className="text-xs text-gray-600 mt-1 inline-block">/{s.slug}</span>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className={clsx(
                  ...
                  configStore?.id === s.id && "ring-1 ring-brand-500/50"
                )}
              >
      ...
      {configStore && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card p-6 max-w-2xl">
          ...
          <p className="text-sm text-gray-500 mb-6">Contexto para el asistente IA y valores por defecto de agenda.</p>
          <div className="space-y-4">
            <div>
              <label className="text-xs text-gray-400 mb-1 block">Contexto del negocio (IA)</label>
              <textarea
                value={businessCtx}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-gray-400 mb-1 block">Tono del asistente</label>
                <input value={tone} onChange={(e) => setTone(e.target.value)} className="input-field" placeholder="professional" />
              </div>
              <div>
                <label className="text-xs text-gray-400 mb-1 block">Duración reunión por defecto (min)</label>
      ...
      {stores?.items?.length === 0 && !showForm && (
        <div className="text-center py-16 glass-card">
          <p className="text-gray-400 mb-4">Aún no tienes tiendas. Crea una para comenzar.</p>
      ...
      <p className="text-xs text-gray-600 mt-10">
        Tienda activa en esta sesión: {getStoreId() ? <span className="text-gray-400 font-mono">{getStoreId()}</span> : "ninguna"}{" "}
        <Link href="/dashboard" className="text-brand-400 hover:underline ml-2">
          Ir al panel
        </Link>
      </p>
    </AppLayout>
